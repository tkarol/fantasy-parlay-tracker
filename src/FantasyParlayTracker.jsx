...
import { onAuthStateChanged, signInWithPopup, signOut, signInWithRedirect } from "firebase/auth";
...
export default function FantasyParlayTracker() {
  const { leagueId } = useParams();
  const activeLeagueId = leagueId || "default"; // fallback if you want legacy to still work
  ...
  useEffect(() => {
    ...
    setIsAdmin(email === "tkarol48@gmail.com");
  }, [user]);
  useEffect(() => {
    if (!isAdmin) { setJoinReqs([]); return; }
    const q = collection(db, "leagues", activeLeagueId, "joinRequests");
    const unsub = onSnapshot(q, (snap) => {
      setJoinReqs(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });
    return unsub;
  }, [isAdmin, activeLeagueId]);
  
  async function approveJoin(uid, displayName, email) {
    if (!isAdmin) return;
    await setDoc(doc(db, "leagues", activeLeagueId), {
      members: {
        [uid]: {
          role: "member",
    ...
    await deleteDoc(doc(db, "leagues", activeLeagueId, "joinRequests", uid));
  }
  ...
  const login = async () => {
    ...
    try {
      ...
      try {
        ...
        setAuthError(err2?.message || "Sign-in failed");
      }
    }
  };
  ...
  // ---- Data access state (avoid popups when signed-out rules block reads)
  ...
  useEffect(() => {
    const qRef = query(
      collection(db, "leagues", activeLeagueId, "weeks"),
      orderBy("season"),
      orderBy("week")
    );
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        // If signed out and your rules block reads, this is expected
        if (!user) {
          setCanRead(false);
          setWeeksList([]);
          setSelectedWeekIdx(-1);
        ...
        // Signed in but still error: keep UI stable and log it
  ...
  useEffect(() => {
    if (!selectedWeek || !canRead) {
      setLegs([]);
      return;
    }
    const legsRef = collection(db, "leagues", activeLeagueId, "weeks", selectedWeek.id, "legs");
    const unsub = onSnapshot(
      legsRef,
    ...
    return unsub;
  }, [selectedWeek, canRead, user?.uid]); // <= added user?.uid
  ...
  const filledLegs = legs.filter((l) => l.odds !== "" && l.odds !== undefined && l.odds !== null && Number.isFinite(Number(l.odds)));
  ...
  // Member can edit only current open week before deadline; admin can always edit
  const memberCanEdit = !!user && (!isAdmin ? (isCurrentWeek && !deadlinePassed && !weekClosed) : true);
  ...
  async function submitOrUpdateMyLeg(e) {
    ...
    if (!isAdmin && (!isCurrentWeek || weekClosed || deadlinePassed)) {
      return alert("Submissions are only allowed for the current open week before the deadline.");
    }
    if (!legText.trim()) return alert("Enter your leg description");

    try {
      const ref = doc(db, "leagues", activeLeagueId, "weeks", wk.id, "legs", user.uid);
      const payload = {
  ...
  async function deleteMyLeg() {
    ...
    if (!wk) return;
    if (!isAdmin && (!isCurrentWeek || weekClosed || deadlinePassed)) return alert("Edits disabled for this week.");
    await deleteDoc(doc(db, "leagues", activeLeagueId, "weeks", wk.id, "legs", user.uid));
  }
  ...
  async function createWeek(season, week, stake = 5, deadlineDate) {
    ...
    const wref = doc(db, "leagues", activeLeagueId, "weeks", id);
    await setDoc(
      wref,
      {
  ...
  async function updateStake(newStake) {
    ...
    await updateDoc(doc(db, "leagues", activeLeagueId, "weeks", selectedWeek.id), {
  ...
  async function deleteWeek() {
    ...
    await deleteDoc(doc(db, "leagues", activeLeagueId, "weeks", selectedWeek.id));
  }
  ...
  const adminAddLeg = useCallback(
    async ({ weekId, memberName, leg, odds, result }) => {
      ...
      const newRef = doc(collection(db, "leagues", activeLeagueId, "weeks", weekId, "legs"));
      await setDoc(
        newRef,
        {
  ...
  const beginEdit = (row) => {
    ...
    setDraftLeg(row.leg || "");
    setDraftOdds(
      row.odds === "" || row.odds === undefined || row.odds === null ? "" : String(row.odds)
    );
  ...
  const saveEdit = async () => {
    ...
    const ref = doc(db, "leagues", activeLeagueId, "weeks", selectedWeek.id, "legs", editingId);
    const payload = {
  ...
  const adminDeleteLeg = async (legId) => {
    ...
    await deleteDoc(doc(db, "leagues", activeLeagueId, "weeks", selectedWeek.id, "legs", legId));
  ...
  async function closeWeekAndCreateNext() {
    if (!isAdmin || !selectedWeek) return;
    const wref = doc(db, "leagues", activeLeagueId, "weeks", selectedWeek.id);
    ...
    if (!nextExists) {
      await setDoc(
        doc(db, "leagues", activeLeagueId, "weeks", nextId),
        {
          season: nextSeason,
          week: nextWeekNum,
  ...
  if (!user && !canRead) {
    return <PublicLanding onSignIn={login} authError={authError} />;
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200 p-2 md:p-4 overflow-x-hidden">
      <div className="w-full">
        {/* HEADER */}
        <header className="rounded-2xl bg-white/90 backdrop-blur border shadow-sm p-3 md:p-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="space-y-1">
            {authError && (
              <div className="mb-2 rounded-lg border border-rose-200 bg-rose-50 text-rose-700 text-sm px-3 py-2">
                {authError}
              </div>
            )}
            <Link to="/" className="text-sm text-slate-600 hover:underline">← Back to Hub</Link>
          ...
          <div className="flex items-center gap-2 md:gap-3">
            {user ? (
              <div className="flex items-center gap-2 rounded-full border bg-slate-50 pl-2 pr-1 py-1">
                ...
                <button onClick={logout} className="rounded-lg bg-slate-900 text-white px-2 md:px-3 py-1 hover:opacity-90 ml-1 md:ml-2 text-xs md:text-sm">
                  Sign out
                </button>
              </div>
            ) : (
              <button onClick={login} className="rounded-xl bg-slate-900 text-white px-3 md:px-4 py-2 text-sm md:text-base shadow hover:opacity-90">
                Sign in
              </button>
            )}
          </div>
        ...
        {/* If we cannot read (likely signed-out & rules require auth): show a friendly card */}
        {!canRead && !user && (
          <section className="mt-3 rounded-2xl border shadow-sm bg-white p-4">
            <h2 className="text-lg font-semibold">Welcome!</h2>
            <p className="text-slate-600 text-sm mt-1">
              Please sign in to view your league weeks and legs.
            </p>
            <button onClick={login} className="mt-3 rounded-lg bg-slate-900 text-white px-4 py-2">
              Sign in with Google
            </button>
          </section>
        )}

        {/* MAIN CONTENT (only show when we can read, or user is signed in) */}
        {(user || canRead) ? (
          <>
            <section className="mt-3 rounded-2xl border shadow-sm bg-white p-3 md:p-5">
              <div className="flex items-center justify-between mb-3 gap-2">
                ...
                <div className="flex items-center gap-1 md:gap-2">
                  <button
                    ...
                    className="rounded-lg border px-2 md:px-3 py-1 disabled:opacity-40"
                    aria-label="Previous week"
                  >
                    ◀
                  </button>
                  <div className="text-xs md:text-sm text-slate-500 tabular-nums">
                    {weeksList.length ? selectedWeekIdx + 1 : 0} / {weeksList.length || 0}
                  </div>
                  <button
                    disabled={selectedWeekIdx >= weeksList.length - 1}
                    onClick={() => setSelectedWeekIdx((i) => Math.min(weeksList.length - 1, i + 1))}
                    className="rounded-lg border px-2 md:px-3 py-1 disabled:opacity-40"
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="text-xs md:text-sm">
                    ...
                    {deadlinePassed && !weekClosed && (
                      <span className="ml-2 text-[10px] md:text-xs px-2 py-0.5 rounded-full border border-rose-200 bg-rose-50 text-rose-700">Deadline passed</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 2xl:grid-cols-12 gap-3 md:gap-4">
                              ...
                              <div className="flex items-start gap-2 md:gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="text-xs md:text-sm text-slate-500 truncate">
                                    {e.memberName || nameFromEmail(e.createdByEmail) || e.id}
                                  </div>
                ...
                {/* Compact combined card */}
                <div className="2xl:col-span-3">
                  <div className="rounded-xl border p-3 md:p-4 bg-slate-50">
                    <div className="text-[10px] md:text-xs uppercase tracking-wide text-slate-500">Parlay</div>
                    <div className="mt-1 text-[24px] md:text-[28px] leading-none font-bold">
              ...
              <div className="mt-3 md:mt-4 flex gap-1 flex-wrap">
                {weeksList.map((w, i) => (
                  <button
                    ...
                    className={`h-2.5 w-7 rounded ${i === selectedWeekIdx ? "bg-slate-900" : "bg-slate-300"}`}
            ...
            {/* Centered member/admin submit card */}
            <section className="mt-3 md:mt-4">
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  {!user && (
                    <span className="text-[10px] md:text-xs text-amber-700 bg-amber-100 border border-amber-200 px-2 py-1 rounded-full">
                      Sign in to submit
                    </span>
                  )}
                  {user && !isAdmin && (!isCurrentWeek || weekClosed || deadlinePassed) && (
                    <span className="text-[10px] md:text-xs text-slate-700 bg-slate-100 border border-slate-200 px-2 py-1 rounded-full">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Steelers ML, Kelce 60+ yards - Not Goedert ATTD (Fuck you Este)"
                    value={legText}
            ...
            <MemberStats selectedSeason={selectedWeek?.season} allWeeks={weeksList} canRead={canRead} />
          </>
        ) : null}
...
/** MemberStats — respects canRead so we don’t error when signed-out */
function MemberStats({ selectedSeason, allWeeks, canRead }) {
  ...
  useEffect(() => {
    ...
    let unsubs = [];
    const weeksForSeason = (season == null ? allWeeks : allWeeks.filter((w) => Number(w.season) === season)) || [];
    if (!weeksForSeason.length) {
      setLegsByWeek({});
      return () => {};
    }
    weeksForSeason.forEach((w) => {
      const legsRef = collection(db, "leagues", activeLeagueId, "weeks", w.id, "legs");
      const unsub = onSnapshot(
        legsRef,
        (snap) => {
          setLegsByWeek((prev) => ({
  ...
  const members = useMemo(() => {
    const set = new Set(
      seasonLegs.map((l) => {
        ...
        if (nm) return nm;
        const guess = (l.createdByEmail && nameFromEmail(l.createdByEmail)) || (l.createdBy && String(l.createdBy)) || l.id;
        return String(guess);
      })
    );
  ...
  return (
    <section className="mt-3 md:mt-4 rounded-2xl border shadow-sm p-3 md:p-5 bg-white">
      <div className="flex items-center justify-between mb-3">
        ...
        <small className="text-slate-500">based on $5 per leg</small>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full text-xs md:text-sm">
          ...
          {sortedMembers.map((m) => {
            const s = byMember[m];
            const resolved = s.wins + s.losses;             // ✅ ignore Pending (and Push)
...
function PublicLanding({ onSignIn, authError }) {
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200">
      <div className="mx-auto max-w-screen-xl px-4 py-10 md:py-16">
        <header className="flex items-center justify-between">
          ...
          <button
            onClick={onSignIn}
            className="rounded-xl bg-slate-900 text-white px-4 py-2 shadow hover:opacity-90"
          >
            Sign in with Google
          </button>
        </header>
        ...
        {authError && (
          <div className="mt-4 rounded-lg border border-rose-200 bg-rose-50 text-rose-700 text-sm px-3 py-2">
            {authError}
          </div>
        )}

        <main className="mt-8 md:mt-12 grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10">
          <section className="rounded-2xl bg-white shadow-sm border p-6 md:p-8">
            <h2 className="text-xl font-semibold text-slate-900">Track your weekly parlay</h2>
            <p className="mt-2 text-slate-600">
              Each member contributes one leg. See combined odds, potential payout, and results over time.
            </p>
            <ul className="mt-4 space-y-2 text-slate-700">
              ...
              <li className="flex items-start gap-2">
                <span className="mt-1 h-2 w-2 rounded-full bg-emerald-500"></span>
                Member stats, streaks, and hit rates
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-1 h-2 w-2 rounded-full bg-emerald-500"></span>
                Admin controls for weeks, odds, and results
              </li>
            </ul>
          </section>

          <section className="rounded-2xl border bg-white/70 backdrop-blur p-6 md:p-8">
            ...
            <div className="mt-3 rounded-xl border bg-slate-50 p-4">
              ...
              <div className="mt-3 rounded-lg border bg-white p-3">
                <div className="text-xs uppercase tracking-wide text-slate-500">Parlay</div>
              ...
              <ul className="mt-3 divide-y">
                <li className="py-3 pl-3 pr-2 flex items-start gap-3 border-l-4 border-amber-500">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="font-medium">Waiting for legs…</div>
                  </div>
